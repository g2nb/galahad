...
class GalaxyToolWidget(UIBuilder):
    ...
    def create_function_wrapper(self, task):
        ...
        def submit_job(**kwargs):
            ...
            display(GalaxyDatasetWidget(job, logo='none', color=session_color(self.task.server_data.url, secondary_color=True)))
    ...
    def create_param_spec(self, task, kwargs):
        ...
        for p in task.params + params_for_job:
            ...
            spec[safe_name]['default'] = GalaxyToolWidget.form_value(
                GalaxyToolWidget.override_if_set(safe_name, 'default', param_overrides, p.get_default_value())
            )
            spec[safe_name]['description'] = GalaxyToolWidget.form_value(
                GalaxyToolWidget.override_if_set(safe_name, 'description', param_overrides, p.description)
            )
    ...
    def __init__(self, task=None, origin='', id='', **kwargs):
        ...
        #         'logo': GALAXY_LOGO,
        #         'origin': origin,
        #         'parameter_groups': GalaxyToolWidget.extract_parameter_groups(self.task),
    ...
    def login_callback(self, data):
        ...
        if self.task is not None and self.task.server_data is None and self.error is not None:
            ...
            with self.output:
                display(GalaxyToolWidget(task, **self.kwargs))
                self.close()
...
def reproduce_job(sessions, session_index, job_number):
    ...
    def create_task():
        ...
        return GalaxyToolWidget(task)                       # Return widget for display
...
def load_tool(sessions, session_index, name_or_lsid):
    ...
    return GalaxyToolWidget(task)                       # Return the task widget
